...
# info:
# 1、创建有调用其他类函数的函数体，包含：方法名、参数列表、函数体
# 2、作用：构建循环调用函数体
...
# 变量定义重复过滤
from Scripts.CreateMixWasteCode import GenerateClass
...
# 3、_generate_class：GenerateClass对象，用于获取可以被调用的信息
# 返回值说明：
# _func_body_content：方法体
# _func_name_base：方法名
# _param_list：参数列表：[{类型：参数名},....]
# _return_type：返回值类型
def generate_function(_name_num, _code_chunk_num, _generate_class):
    ...
    _func_body = get_func_body(_return_type, _param_list, _generate_class)
...
def get_func_body(_return_type, _param_list, _generate_call):
        ...
        for _param_dict in _param_list:
            for _param_type in _param_dict.keys():
                ...
                if 'List<int>' in _param_type:
                    _code_chunk = get_func_body_for_list_int(_param_dict[_param_type], _generate_call) + '\n'
                elif 'List<string>' in _param_type:
                    _str_func_random = random.randint(0, 2)
                    if _str_func_random == 1:
                        _code_chunk, _return_list_int_val = get_func_body_for_list_string_1(_param_dict[_param_type], _generate_call)
                    else:
                        _code_chunk, _return_list_str_val = get_func_body_for_list_string_2(_param_dict[_param_type], _generate_call)
                elif 'bool' not in _param_type:
                    _code_chunk, _return_int_val = get_func_body_for_int(_param_dict[_param_type], _generate_call)
                _func_body += _code_chunk + '\n'
    else:
        _str_func_body, _return_int_val = get_func_body_for_int(None, _generate_call)
    ...
    if _return_type == 'void':
        ...
        _func_body += get_return_int_value(_return_int_val, _return_list_int_val, _generate_call)
    elif _return_type == 'float':
        _func_body += get_return_float_value(_return_int_val, _return_list_int_val, _generate_call)
    elif _return_type == 'string':
        _func_body += get_return_string_value(_return_int_val, _return_list_int_val, _return_list_str_val, _generate_call)
    elif _return_type == 'bool':
        _func_body += get_return_bool_value()
    elif _return_type == 'List<string>':
        _func_body += get_return_list_string_value(_return_int_val, _return_list_int_val, _return_list_str_val, _generate_call)
    elif _return_type == 'List<int>':
        _func_body += get_return_list_int_value(_return_int_val, _return_list_int_val, _generate_call)
    return _func_body


# 得到list<int>对应的函数体
# _list_int_param_name: 参数List<int>类型的变量名
def get_func_body_for_list_int(_list_int_param_name, _generate_call):
    _variable_name = get_random_variable_name()
    _str_0 = str.format('\t\t\tif ({0} != null) {1}\n', _list_int_param_name, '{')
    _str_1 = str.format('\t\t\t\tint {0} = {1};\n', _variable_name, _generate_call.get_no_param_func('int'))
...
def get_func_body_for_list_string_1(_list_string_param_name, _generate_call):
    _variable_name = get_random_variable_name()
    _variable_name_1 = get_random_variable_name()
    _str_0 = str.format('\t\t\tint {0}={1};\n', _variable_name, _generate_call.get_no_param_func('int'))
    _str_1 = str.format('\t\t\tif ({0} != null) {1}\n', _list_string_param_name, '{')
    _str_2 = str.format('\t\t\t\tfor (int i = 0; i < {0}.Count; i++) {1}\n', _list_string_param_name, '{')
    _str_3 = str.format('\t\t\t\t\tint {0} = {1};\n', _variable_name_1, _generate_call.get_one_param_func('int'))
...
# _list_int_param_name: 参数List<string>类型的变量名
def get_func_body_for_list_string_2(_list_string_param_name, _generate_call):
    _variable_name = get_random_variable_name()
    _variable_name_2 = get_random_variable_name()
    _str_0 = str.format('\t\t\tstring {0} = {1};\n', _variable_name, _generate_call.get_no_param_func('string'))
    _str_1 = str.format('\t\t\tif ({0} != null) {1}\n', _list_string_param_name, '{')
    _str_2 = str.format('\t\t\t\tfor (int i = 0; i < {0}.Count; i++) {1}\n', _list_string_param_name, '{')
...
def get_func_body_for_int(_param_name, _generate_call):

    _str_2_1 = ''
    if _param_name is None:
        _param_name = get_random_variable_name()
        _str_2_1 = str.format('\t\t\tstring {0} = {1};\n', _param_name, _generate_call.get_one_param_func('string'))

    _variable_name = get_random_variable_name()
    _variable_name_1 = get_random_variable_name()

    _str_1 = str.format('\t\t\tint {0}= {1};\n', _variable_name, _generate_call.get_no_param_func('int'))
    _str_2 = str.format('\t\t\tint {0} = {1};\n', _variable_name_1, _generate_call.get_one_param_func('int'))
...
# region 得到函数返回值
# 得到float类型的返回值
def get_return_float_value(_val_1, _val_2, _generate_call):
    _value = ''
    if random.randint(0, 2) == 0 and _val_1 != '':
        ...
        _value = str.format('\t\t\treturn {0};\n', _generate_call.get_one_param_func('float'))
    return _value


# 得到int类型的返回值
def get_return_int_value(_val_1, _val_2, _generate_call):
    _value = ''
    if random.randint(0, 2) == 0 and _val_1 != '':
        _value = str.format('\t\t\treturn {0};\n', _val_1)
    elif _val_2 != '':
        ...
        _value = str.format('\t\t\treturn {0};\n', _generate_call.get_no_param_func('int'))
    return _value


# 得到List<int> 类型的返回值
def get_return_list_int_value(_val_1, _val_2, _generate_call):
    _value = ''
    if random.randint(0, 2) == 0 and _val_1 != '':
...
def get_return_list_string_value(_val_1, _val_2, _val_3, _generate_call):
...
# 得到string类型的返回值
def get_return_string_value(_val_1, _val_2, _val_3, _generate_call):
    _random_val = random.randint(0, 3)
    _value = ''
    if _random_val == 0 and _val_1 !=  '':
        ...
        _value = str.format('\t\t\treturn {0};\n', _generate_call.get_one_param_func('string'))
    return _value
# endregion
